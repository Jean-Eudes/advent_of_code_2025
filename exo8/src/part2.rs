...
impl<'a> Circuit<'a> {
    ...
    fn add_point(&mut self, point: &'a Point) {
...
fn main() {
    ...
    let mut circuits = Circuits::new(&points);
    for edge in edges.iter() {
        circuits.add_edge(edge);
        if circuits.circuits.len() == 1 {
            println!("{:?}", edge.point1.x * edge.point2.x);
            break;
        }
    }
}
